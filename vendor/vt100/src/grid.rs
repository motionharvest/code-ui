...
impl Grid {
    ...
    pub fn col_wrap(&mut self, width: u16, wrap: bool) {
        if self.pos.col > self.size.cols - width {
            ...
            // `scrolled` can exceed `prev_pos.row` when the cursor row was
            // already out of bounds for the scroll region; plain subtraction
            // overflows in debug builds (seen with some full-screen TUIs).
